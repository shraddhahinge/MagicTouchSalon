import { Box, Image, Heading, Text, Grid } from "theme-ui";
...
const FavoriteCard = (props) => {
  return (
    <Box sx={styles.fevCard}>
      <Link
        sx={styles.fevCard.image}
        path={props?.navigate ? "/service" : "/#service"}
        onClick={() => {
          localStorage.setItem("selectedService", JSON.stringify(props?.list));
        }}
      >
      ...
      <Box sx={styles.fevCard.content}>
        <Box sx={styles.fevCard.content.heading}>
        ...
        <Grid gap={2} columns={2}>
          {props?.list?.map(({ id, title }) => (
            <Box sx={styles.pointsLayout} key={id}>
              <Image src={scissor} alt="points image" />
...
const styles = {
  fevCard: {
    ...
    content: {
      ...
      h3: {
        fontSize: ["18px", null, null, "20px", null, "20px", "20px"],
        lineHeight: 1.5,
        color: "black",
        margin: 0,
        fontWeight: 700,
